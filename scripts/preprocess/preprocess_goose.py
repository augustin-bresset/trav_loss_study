...
Usage:
    python -m scripts.preprocess.preprocess_goose --config resources/goose_preprocess.yaml
    python -m scripts.preprocess.preprocess_goose --config resources/goose_preprocess.yaml --split val
    python -m scripts.preprocess.preprocess_goose --config resources/goose_preprocess.yaml --overwrite
    python -m scripts.preprocess.preprocess_goose --config resources/goose_preprocess.yaml --skip-gicp
"""
...
from apairo_preprocess import GICPOdometry, TraversabilityFromTrajectory
...
def run_split(split_dir: Path, cfg: dict, *, overwrite: bool, skip_gicp: bool) -> None:
    ...
    # ------------------------------------------------------------------ GICP
    if not skip_gicp:
        log.info("Step 1/2  GICPOdometry")
        gicp = GICPOdometry(
            voxel_size=cfg["gicp"]["voxel_size"],
            max_range=cfg["gicp"]["max_range"],
        ...
        log.info("Step 1/2  GICPOdometry — skipped (--skip-gicp)")
    ...
    log.info("Step 2/2  TraversabilityFromTrajectory")
    trav_traj = TraversabilityFromTrajectory(
        poses=all_poses,
