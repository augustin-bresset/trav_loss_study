"""Compare loss functions on GOOSE-3D traversability data.

Runs one training subprocess per loss, then aggregates TensorBoard results
into data/goose_study_results.{json,csv}.

Usage:
    python -m src.scripts.study_goose                           # all losses
    python -m src.scripts.study_goose --losses bce nnpu        # subset
    python -m src.scripts.study_goose --config resources/train_goose.yaml
    python -m src.scripts.study_goose --dry-run
"""
...
def main() -> None:
    ...
    results_json = out_dir / "goose_study_results.json"
    results_csv  = out_dir / "goose_study_results.csv"
